...
(define-public (set-max-pools (new-max uint))
  (begin
    (asserts! (> new-max u0) (err ERR-MAX-GROUPS-EXCEEDED))
...
(define-public (create-pool
  ...
  (let (
    ...
    (asserts! (is-none (map-get? pools-by-name pool-name)) (err ERR-GROUP-ALREADY-EXISTS))
      ...
      g
        (begin
          ...
          (let ((existing (map-get? pools-by-name update-name)))
            (match existing
              existing-id
                (asserts! (is-eq existing-id pool-id) (err ERR-GROUP-ALREADY-EXISTS))
              (begin true)
            )
          )
